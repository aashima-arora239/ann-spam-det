...
public class MainHam {

    static List<MailMessage> hamList = new ArrayList<MailMessage>();
    static List<Set<MailDTO>> hamIndex = new ArrayList<Set<MailDTO>>();
    ...
    public static void LoadMails() throws Exception {
        MainHam.CreateHamIndex();
        Runnable r = new Runnable() {

            @Override
            public void run() {
                MainHam.SetTermDTO();  //Setting term Index - Thread 1
            }
        };
        Runnable r1 = new Runnable() {
            ...
            @Override
            public void run() {
                MainHam.ConstructPostingList(); //Constructing posting list - Thread 2
        ...
        Runnable r2 = new Runnable() {
            ...
            @Override
            public void run() {
                MainHam.SetVector(); //Setting mail vectors - Thread 3
            }
        };
        ...
        try {
            ...
            if (f1.exists()) {
                ...
                in.close();
                Backpropogation.performBackprop(feature,updatedWeights,0);
            } else {
                Backpropogation.performBackprop(feature,weights,0);
            }
    ...
    public static void CreateHamIndex() throws IOException {
        hamList = Utils.ReadHamMail();
        System.out.println("Read Mail Called");
        hamIndex = Utils.TokenizeMail(hamList);
        System.out.println("hamIndex created");

    }

    public static void SetTermDTO() {
        Iterator<Set<MailDTO>> hamIndexIterator = hamIndex.iterator();


        while (hamIndexIterator.hasNext()) {
            Set<MailDTO> setVal = hamIndexIterator.next(); //Retrieve set from hamIndex
    ...
    public static void ConstructPostingList() {
        ...
        while (termIterator.hasNext()) {
            ...
            Iterator<Set<MailDTO>> hamIndexIterator = hamIndex.iterator();
            //Iterate over hamIndex to retrieve a set and iterate over that set to match the term index.
            while (hamIndexIterator.hasNext()) {
                Set<MailDTO> setVal = hamIndexIterator.next();
                Iterator<MailDTO> mailVal = setVal.iterator();
                while (mailVal.hasNext()) {
                    MailDTO mail = mailVal.next();
                    if (mail.getTerm().equals(termValue.getTerm())) {
                        mails.add(mail); //Update the hamList if term found.
                    }
